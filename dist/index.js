...
var ssoClientSdk = require('@tjsglion/sso-client-sdk');
var ssoIcons = require('@tjsglion/sso-icons');
...
const useAuth = (config) => {
    ...
    const ssoClient = new ssoClientSdk.SSOClient({
        baseUrl: config.apiUrl,
        redirectUri: config.redirectUrl,
    });
    const checkAuth = require$$0.useCallback(async () => {
        try {
            setLoading(true);
    ...
    const login = require$$0.useCallback(async (credentials) => {
        try {
            setLoading(true);
    ...
    const register = require$$0.useCallback(async (credentials) => {
        try {
            setLoading(true);
            if (credentials.email && credentials.password) {
    ...
    const loginWithSSO = require$$0.useCallback(async (provider) => {
        try {
            setLoading(true);
    ...
    const verify2FA = require$$0.useCallback(async (code) => {
        try {
            setLoading(true);
...
const TwoFactorAuth = ({ onVerify, onCancel, theme, className = '', }) => {
    ...
    const [countdown, setCountdown] = require$$0.useState(0);
    // 倒计时效果
    require$$0.useEffect(() => {
        if (countdown > 0) {
...
const TwoFactorSetup = ({ qrCodeUrl, secretKey, onEnable, onCancel, theme, className = '', }) => {
    const [verificationCode, setVerificationCode] = require$$0.useState('');
    const [step, setStep] = require$$0.useState('qr');
    const handleVerify = () => {
...
const PhoneAuth = ({ phone, code, onSendCode, onVerify, theme, className = '', }) => {
    const [phoneNumber, setPhoneNumber] = require$$0.useState(phone);
    const [verificationCode, setVerificationCode] = require$$0.useState(code);
    const [countdown, setCountdown] = require$$0.useState(0);
    const [step, setStep] = require$$0.useState('phone');
    // 倒计时效果
    require$$0.useEffect(() => {
        if (countdown > 0) {
            const timer = setTimeout(() => setCountdown(countdown - 1), 1000);
...
const UnifiedAuthForm = ({ config, theme, onLoginSuccess, onLoginError, defaultMethod = 'email', showMethodSwitch = true, methodOrder = ['email', 'phone', 'sso'], className = '', }) => {
    const [currentMethod, setCurrentMethod] = require$$0.useState(defaultMethod);
    const [loginCredentials, setLoginCredentials] = require$$0.useState({});
    const [show2FA, setShow2FA] = require$$0.useState(false);
    // 获取启用的认证方式
...
exports.SSOButtons = SSOButtons;
exports.TwoFactorAuth = TwoFactorAuth;
exports.TwoFactorSetup = TwoFactorSetup;
exports.UnifiedAuthForm = UnifiedAuthForm;
